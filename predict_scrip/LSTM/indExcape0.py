...
parser.add_argument("-availableGPUs", help="Available GPUs",
                    nargs='*', type=int, default=[0])
parser.add_argument("-smiles_file", help="smiles needed to be predicted",
                    type=str, default=basePath+'/data/test.csv')
parser.add_argument("-method_name", help="method name",
                    type=str, default='LSTM')
parser.add_argument("-datatset_type", help="dataset type",
                    type=str, default='chembl29')
parser.add_argument("-saveBasePath", help="saveBasePath",
                    type=str, default=basePath+'/result/')
...
dataPathSave1 = basePath+'/model/'+datatset_type + \
    '/'+datatset_type+'_original_data/'
...
for i in range(smiles_number):
    ...
    fp_1 = AllChem.GetMorganFingerprintAsBitVect(
        mol_id, 3, 2048, useFeatures=True)
...
sampleECFPInd = pd.Series(data=np.arange(
    len(smiles_list)), index=np.arange(len(smiles_list)))
...
if ("lstmSmilesInputData" in globals()) and (not (lstmSmilesInputData is None)):
    testSmilesLSTMInput = np.array(lstmSmilesInputData)[
        lstmSampleIndex[testSamples].values].copy()
    testGrtaphLSTMInput = np.array(lstmGraphInputData)[
        lstmSampleIndex[testSamples].values].copy()
    ...
    testLSTMSideOutputECFP = testLSTMSideOutputECFP[:, np.argsort(
        -testLSTMSideOutputECFP.sum(0).A[0])[0:128]].A
    testLSTMSideOutputFCFP = testLSTMSideOutputFCFP[:, np.argsort(
        -testLSTMSideOutputFCFP.sum(0).A[0])[0:128]].A

    testLSTMSideOutput = np.hstack(
        [testLSTMSideOutputECFP, testLSTMSideOutputFCFP, testLSTMSideOutputMACCS])
...
print(predMatrix)
predMatrix.to_csv(saveBasePath+datatset_type+method_name +
