...
dictionary0 = {
    'bernoulliModel_alpha':np.arange(0.1,1.1,0.1)
}
#Root directory
basePath=os.getcwd()
...
def data_split_modeling(target_file):
    ...
    for outerFold in compOuterFolds:
        ...
        for innerFold in compInnerFolds:
            ...
            for paramNr in range(0, hyperParams0.shape[0]):
                pperf.append(save_path + "o" + '{0:04d}'.format(outerFold + 1) + "_i" + '{0:04d}'.format(
                    innerFold + 1) + "_p" + '{0:04d}'.format(hyperParams0.index.values[paramNr]) + ".test.auc.pckl")
    ...
    bernoulliModel_alpha1 = hyperParams0.iloc[paramNr].bernoulliModel_alpha
    nb = BernoulliNB(alpha=bernoulliModel_alpha1)
