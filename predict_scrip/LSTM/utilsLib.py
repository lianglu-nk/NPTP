...
def calculateF1s(t, p):
  ...
  for i in range(p.shape[1]):
    ...
    idx = np.abs(t[:, i]) > 0.5
    try:
      aucs.append(sklearn.metrics.f1_score(targ[idx], pred[idx]))
    except ValueError:
      aucs.append(np.nan)
  return aucs
    
def calculateKAPPAs(t, p):
  aucs = []
  for i in range(p.shape[1]):
    targ = t[:, i] > 0.5
    pred = p[:, i] > 0.5
    idx = np.abs(t[:, i]) > 0.5
    try:
      aucs.append(sklearn.metrics.cohen_kappa_score(targ[idx], pred[idx]))
    except ValueError:
      aucs.append(np.nan)
  return aucs
